    ...
    return isFocus ? (
      <div>
        ...
        >
          {tempSuggestedQuestions.map((question, index) => {
            return (
              <div className='group relative    border border-gray-200 flex items-center pl-2.5 hover:border-gray-300 hover:bg-white' key={index}>
                ...
                <input
                  ...
                  className={'w-full overflow-x-auto pl-1.5 pr-8 text-sm leading-9 text-gray-900 border-0 grow h-9 bg-transparent focus:outline-none cursor-pointer   '}
                />

                <div
                  className='block absolute top-1/2 translate-y-[-50%] right-1.5 p-1     cursor-pointer hover:bg-[#FEE4E2] hover:text-[#D92D20]'
                  onClick={() => {
        ...
        {tempSuggestedQuestions.length < MAX_QUESTION_NUM && (
          <div
            ...
            className='mt-1 flex items-center h-9 px-3 gap-2    cursor-pointer text-gray-400  bg-gray-100 hover:bg-gray-200'>
      ...
      <div className='mt-1.5 flex flex-wrap'>
        {notEmptyQuestions.map((question, index) => {
          return (
            <div key={index} className='mt-1 mr-1 max-w-full truncate last:mr-0 shrink-0 leading-8 items-center px-2.5    border border-gray-200 shadow-xs bg-white text-[13px] font-normal text-gray-900 cursor-pointer'>
              {question}
            </div>
          )
        })}
