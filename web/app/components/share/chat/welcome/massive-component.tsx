...
export const AppInfo: FC<{ siteInfo: SiteInfo }> = ({ siteInfo }) => {
  return (
    <div>
      <div className='flex items-center py-2 text-xl font-medium text-gray-700    '>{siteInfo.icon && <span className='mr-2'><em-emoji id={siteInfo.icon} /></span>}{siteInfo.title}</div>
...
export const EditBtn = ({ className, onClick }: { className?: string; onClick: () => void }) => {
  ...
  return (
    <div
      className={cn('px-2 flex space-x-1 items-center      cursor-pointer', className)}
      onClick={onClick}
    >
